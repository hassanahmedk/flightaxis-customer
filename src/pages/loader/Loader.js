		...
		getFlights()
			.then((flights) => {
				flights = flights.filter((flight) => {
					if (
						flight.leaving_from === formData.leaving_from &&
						flight.going_to === formData.going_to
					) {
						return flight;
					}
					if (
						flight.leaving_from === formData.going_to &&
						flight.going_to === formData.leaving_from
					) {
				...
				if (state.type === "one-way") {
					navigate("/search-results-one-way", {
						state: { flights, formData: formData },
					});
				} else {
					navigate("/search-results-round-trip", {
						state: { flights, formData: formData },
					});
				}
			})
			.catch((e) => {
				window.alert(`Cannot get flights, ${e}`);
			});
	}, []);
